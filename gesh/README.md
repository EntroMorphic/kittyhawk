...
**Phase A.2 — lattice update training — ONLINE.** Coordinate-descent flips on projection trits to reduce loss. No STE. The lattice IS the geometry. **Measured across a multi-seed sig_dim sweep extended through sig_dim = 1024 (`docs/sweep_dims_results.md`, 5 seeds per cell, 12 dims, permille precision): lattice update earns its complexity in the compression regime — gain plateaus at +8pp at sig_dim = 32. Random ternary projection at sig_dim = D beats identity by +7pp robustly across seeds — *mechanism confirmed* via Phase B Gate 2 (`docs/phase_b_gate2_results.md`, Pearson r = +0.89). Expansion saturates monotonically: at sig_dim = 1024 (16× input dim) random and trained converge to 98.8% ± 0.5pp with +0.0pp gain. Capacity floor at sig_dim ≤ 4 is *mechanism confirmed* via Finding 3 high-seed probe (`docs/finding3_high_seed_results.md`): pigeonhole-forced collisions produce only 4 distinct tile signatures for 10 classes at sig_dim = 2.**

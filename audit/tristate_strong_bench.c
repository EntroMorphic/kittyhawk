 ...
 *
 * Runs four NEON-only kernels:
 *   Path A  base3_packed_matmul_neon         — base-3 ternary via SDOT
 *   Path B  b2b_honest_matmul_neon           — B2-B sign+mask separate decode
 *   Path B' b2b_skip_matmul_neon             — B2-B with all-masked-block skip
 *   Path C  b2b_optimal_matmul_neon          — B2-B unified TBL decode (RG-1)
 *
 * Plus external grounding (R-G2) via the substrate's
...
int main(void) {
    printf("config_idx,K,w_zero,a_zero,seed,"
           "ok_a_eq_b,ok_a_eq_skip,ok_a_eq_optimal,ok_a_eq_substrate,"
           "skip_rate,"
           "ms_a,ms_b,ms_skip,ms_optimal,ms_substrate\n");
    ...
    for (int c = 0; c < N_CONFIGS; c++) {
        ...
        double sum_ms_optimal = 0, sum_ms_substrate = 0;
        ...
        for (int s = 0; s < N_SEEDS; s++) {
            ...
            free(Ya); free(Yb); free(Yk); free(Yo); free(Ys);
        }

        double cfg_skip_rate = (double)cfg_skip_blocks / (double)cfg_blocks;
        fprintf(stderr,
            "[summary] cfg %d K=%d w_z=%.2f a_z=%.2f | "
            "verify(/5): a==b:%d a==skip:%d a==opt:%d a==sub:%d | "
            "skip_rate=%.6f | "
            "mean ms (over %d reps × %d seeds): "
            "A=%.2f B=%.2f Bskip=%.2f Bopt=%.2f Sub=%.2f | "
            "B/A=%.2fx Bskip/A=%.2fx Bopt/A=%.2fx Sub/A=%.2fx\n",
            c, K, cfg->w_zero, cfg->a_zero,
            sum_ok_a_eq_b, sum_ok_a_eq_skip, sum_ok_a_eq_optimal, sum_ok_a_eq_substrate,
            cfg_skip_rate,
            REPS, N_SEEDS,
    ...
    fprintf(stderr,
        "[overall] %d runs total | "
        "verify a==b:%d/%d a==skip:%d/%d a==opt:%d/%d a==sub:%d/%d | "
        "total skip rate: %lld/%lld = %.6f\n",
        total_runs,

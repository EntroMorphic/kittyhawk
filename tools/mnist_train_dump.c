 ...
 *
 * Usage: ./mnist_train_dump <mnist_dir> <output_weights.bin> [epochs]
 *
 * Trains for the given number of epochs (default 20), saves all model
 * weights to the output file whenever accuracy improves. The file on
 * disk is always from the best epoch.
 */
...
static void save_weights(const char* path,
    ...
    TrixTernaryRoutedFFN* ffn, TrixAtomFFN* head)
{
    ...
    int w1_elems = ffn->cfg.tile_hidden * D;
    int w2_elems = D * ffn->cfg.tile_hidden;
    for (int t = 0; t < T; t++) {
        float* sw = ffn->W1 + t * w1_elems;
        float mean_abs = 0;
        for (int i = 0; i < w1_elems; i++) mean_abs += fabsf(sw[i]);
        mean_abs /= (float)w1_elems;
        float thresh = mean_abs * 0.5f;
        int8_t* tern = malloc(w1_elems);
        for (int i = 0; i < w1_elems; i++)
            tern[i] = (sw[i] > thresh) ? 1 : (sw[i] < -thresh) ? -1 : 0;
        int32_t c = w1_elems;
        fwrite(&c, 4, 1, fout);
        ...
        sw = ffn->W2 + t * w2_elems;
        mean_abs = 0;
        for (int i = 0; i < w2_elems; i++) mean_abs += fabsf(sw[i]);
...
int main(int argc, char** argv) {
    ...
    float* dh1_b=calloc(batch*H_proj,sizeof(float));
    float* dz1_b=calloc(batch*H_proj,sizeof(float));
    float* dW1_b=calloc(H_proj*input_dim,sizeof(float));
    float* db1_g=calloc(H_proj,sizeof(float));
    float* dW2_b=calloc(D*H_proj,sizeof(float));
    ...
    printf("Training %d epochs (D=%d T=%d K=%d)...\n", n_epochs, D, T, K);
    for (int ep=0; ep<n_epochs; ep++) {
        ...
        for(int b=0;b+batch<=nt;b+=batch){
            ...
            memset(dW2_b,0,D*H_proj*sizeof(float));
            memset(db2_g,0,D*sizeof(float));
            trix_matmul_at(dW2_b,dpo,h1,batch,D,H_proj);
            trix_bias_grad(db2_g,dpo,batch,D);
            trix_matmul(dh1_b,dpo,pW2,batch,D,H_proj);
            trix_gelu_grad(dz1_b,dh1_b,z1,batch*H_proj);
            memset(dW1_b,0,H_proj*input_dim*sizeof(float));
            memset(db1_g,0,H_proj*sizeof(float));
            trix_matmul_at(dW1_b,dz1_b,xb,batch,H_proj,input_dim);
            trix_bias_grad(db1_g,dz1_b,batch,H_proj);
            trix_sgd_update(pW1,dW1_b,lr,H_proj*input_dim);
            trix_sgd_update(pb1,db1_g,lr,H_proj);
            trix_sgd_update(pW2,dW2_b,lr,D*H_proj);
        ...
        for(int o=0;o<nv;o+=batch){
            ...
            trix_matmul_bt(z1,xv+o*input_dim,pW1,n,input_dim,H_proj);
        ...
        printf("  ep %2d: %.2f%% (%d/%d)%s\n", ep, acc*100, cor, nv,
               is_best ? " << BEST — saving weights" : "");

        if (is_best) {
            best_acc = acc;
            best_ep = ep;
            save_weights(output_path, input_dim, D, T, K, H_proj, n_classes,
                         pW1, pb1, pW2, pb2, ffn, head);
        }
    }

    printf("\nBest: %.2f%% at epoch %d → saved to %s\n", best_acc*100, best_ep, output_path);

    free(pW1);free(pb1);free(pW2);free(pb2);
    free(z1);free(h1);free(po);free(fo);free(lo);
    free(dl);free(dfo);free(dpo);free(dh1_b);free(dz1_b);
    free(dW1_b);free(db1_g);free(dW2_b);free(db2_g);

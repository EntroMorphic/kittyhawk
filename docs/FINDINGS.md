...
- **Substrate-distinct claim refined**: direction-aware sparse attention beats direction-blind sparse attention (NOT substrate-specific — base-2 can implement posracle). The substrate's specific contribution is **direction-awareness as a native representation property** — trit signatures bake direction into the data structure, so the routing primitive is direction-aware by construction rather than by post-hoc filter. The remaining ~1-prompt gap (routed > posracle) is small enough to be noise at the test sample size.

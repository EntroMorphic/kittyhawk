...
**Surprise result: routed at k=4 (22/24) outperforms dense (19/24)** on three loop-prone prompts (`code_comment`, `edge_question`, `edge_repetitive`). Mechanism (refined per `journal/loop_regularizer_atomics_2026-05-10.md`): **sparsity itself** is the loop-breaking mechanism — dense's softmax distributes mass across all positions, the mid-weight positions cumulatively reinforce loop patterns; aggressive sparsification (random/oracle/routed alike) cuts this off. The substrate-distinct contribution is **coherent sparse attention via signature similarity** — random sparsity breaks loops but produces incoherent garbage; routed sparsity produces coherent output because signature-distance selection is relevance-aware. The "routed > oracle on full battery" gap (22/24 vs 15/24 at k=4) is the genuinely substrate-distinct signal — both are relevance-aware but routed's signature metric beats oracle's score-magnitude metric on this workload (mechanism for THAT undertheorized).

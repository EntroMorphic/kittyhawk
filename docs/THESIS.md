...
3. **Substrate distance finding**: Go probe RED at first pass (raw Hamming phase-ID 40.40%, density-only 98.28%). Follow-up `substrate_distance_refinement` cycle identified that raw int8 trit Hamming has a **density-scaling bias** — `hamming_norm(a, b) = H · 1024 / (|a|₀ + |b|₀ + 1)` fixes it. Phase-ID lifts to 85.40% (position-split) / **88.40%** (game-split — red-team confirmed no within-game leakage effect). Raw Hamming also already achieved **413× random lift on same-game retrieval** (density-controlled task), showing the substrate was never structurally blind. See `journal/substrate_distance_refinement_closeout.md`.

4. **Image-pipeline measurement (decisive gate, 2026-04-24):** the `hamming_norm` retrofit was tested on MNIST / Fashion-MNIST / CIFAR-10 raw-intensity quantized signatures (full 10k test per dataset, `tools/image_distance_probe.c`). **Result: decisively NEGATIVE.** MNIST tied (95.65% → 95.49%, within noise), Fashion-MNIST hurt by 2.65pp (81.04% → 78.39%), CIFAR-10 marginal gain (13.06% → 14.04%, ~3× noise). `hamming_norm` does NOT retrofit into `direct_lsh` or any image consumer. It is a domain-specific remedy: it fixes raw Hamming when the sparse-vector attractor dominates positional discrimination (Go, where density variance is 50× across phases on a 361-trit signature), and hurts when density correlates informatively with class OR when the pipeline already decorrelates density via normalize. See `journal/hamming_norm_on_images.md`.

**What this changes for the thesis:** the falsification framing in §2 remains intact. The MVP's expert-collapse finding is evidence that *frozen-gate* routing is insufficient for multi-class problems — not that *routing itself* is insufficient. The `step_change` cycle's **CIFAR representation tax is confirmed real, not a metric artifact** — `hamming_norm` does not close it on images. The image-canon demotion from primary benchmark to regression-guard holds firmly. For Go and analogous sparse-discrete domains (chess/othello state, DNA mutation maps, edit tags, directional signals), `hamming_norm` is the correct substrate distance — but it does NOT generalize beyond that class. The `routed_go` trainer cycle can proceed using `hamming_norm` as its distance primitive; the substrate-wide claim from `substrate_distance_refinement_closeout.md` is explicitly rescinded in favor of a domain-specific framing. The "end-game unknowable" clause from NORTH_STAR continues to hold.

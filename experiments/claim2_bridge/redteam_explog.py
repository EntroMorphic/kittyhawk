...
def main():
    ...
    # exp(0) vs 1: fp_encode(1.0) trit-encoding ≠ base_sig_const(1)'s all-+1
    # special case. Remaining design tension between fp and element-wise
    # encodings for the same value.
    ok, msg = check_match("exp(0)", "1", False, "exp(0)-vs-int1 (design)")
    print(msg)
    ...
    for n in [10, 20, 30, 50]:
        try:
            ...
            got = decode_fp_sig(s)
            want = math.exp(n)
            err = abs(got - want) / max(abs(want), 1e-30) if not math.isinf(want) else float('inf')
            status = "OK" if err < 1e-10 else f"err={err:.2e}"
            print(f"  exp({n:>3}) = {got:>22.6g}  want {want:>22.6g}  {status}")
        except Exception as e:

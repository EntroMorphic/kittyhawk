/*
 * glyph_mtfp.c — MTFP arithmetic core
 *
...
void glyph_mtfp_vec_zero(glyph_mtfp_t* x, int n) {
    memset(x, 0, (size_t)n * sizeof(glyph_mtfp_t));
}

/* NEON saturation note: MAX_VAL = INT32_MAX/2, so the sum of two in-range
 * cells cannot wrap int32; we only need to clamp to ±MAX_VAL. Plain
 * vaddq_s32 followed by vminq/vmaxq against splats of ±MAX_VAL is
 * sufficient and cheaper than vqaddq_s32 + clamp. */

void glyph_mtfp_vec_add(
    glyph_mtfp_t* dst, const glyph_mtfp_t* a, const glyph_mtfp_t* b, int n)
{
    int i = 0;
#if GLYPH_HAS_NEON
    const int32x4_t vmax = vdupq_n_s32( GLYPH_MTFP_MAX_VAL);
    const int32x4_t vmin = vdupq_n_s32(-GLYPH_MTFP_MAX_VAL);
    for (; i + 4 <= n; i += 4) {
...
#endif
    for (; i < n; i++) dst[i] = glyph_mtfp_add(a[i], b[i]);
}
...
#endif
    for (; i < n; i++) dst[i] = glyph_mtfp_add(dst[i], a[i]);
}

void glyph_mtfp_vec_scale(
    glyph_mtfp_t* dst, const glyph_mtfp_t* src, glyph_mtfp_t scale, int n)
{
    ...
    for (int i = 0; i < n; i++) {
        dst[i] = glyph_mtfp_mul(src[i], scale);
    }
}
...
static void matmul_row(
    glyph_mtfp_t* Y_row, const glyph_mtfp_t* X_row, const glyph_mtfp_t* W,
    int K, int N)
{
    for (int j = 0; j < N; j++) {
        int64_t acc = 0;
        ...
        if (acc >= 0) acc += GLYPH_MTFP_SCALE / 2;
        else          acc -= GLYPH_MTFP_SCALE / 2;
        Y_row[j] = glyph_mtfp_clamp64(acc / GLYPH_MTFP_SCALE);
    }
}
...
{
    for (int j = 0; j < N; j++) {
        const glyph_mtfp_t* wj = W + (size_t)j * K;
        int64_t acc = 0;
        for (int k = 0; k < K; k++) {
...
void glyph_mtfp_matmul(
    glyph_mtfp_t* Y, const glyph_mtfp_t* X, const glyph_mtfp_t* W,
    int M, int K, int N)
{
    assert(Y && X && W);
    assert(M >= 0 && K >= 0 && N >= 0);
...
void glyph_mtfp_bias_add(
    glyph_mtfp_t* x, const glyph_mtfp_t* b, int batch, int dim)
{
    for (int i = 0; i < batch; i++) {
        glyph_mtfp_vec_add_inplace(x + (size_t)i * dim, b, dim);
    }
}

void glyph_mtfp_fan_in_normalize(glyph_mtfp_t* x, int n, int fan_in) {
    int64_t norm = glyph_isqrt64((int64_t)fan_in);
    if (norm < 1) norm = 1;
    for (int i = 0; i < n; i++) {
        x[i] = (glyph_mtfp_t)(x[i] / norm);
    }
}
...
int64_t glyph_mtfp_isqrt_inv(int64_t x) {
    if (x <= 0) return (int64_t)GLYPH_MTFP_SCALE;
    int64_t s = glyph_isqrt64(x);
    if (s <= 0) return (int64_t)GLYPH_MTFP_SCALE;
    int64_t S = (int64_t)GLYPH_MTFP_SCALE;
    return (S * S) / s;
}
...
static void layernorm_row(
    glyph_mtfp_t* yi, const glyph_mtfp_t* xi,
    const glyph_mtfp_t* weight, const glyph_mtfp_t* bias,
    int64_t eps_var, int cols)
{
    ...
    /* rstd in SCALE units. */
    int64_t rstd = glyph_mtfp_isqrt_inv(var_eps);
    int64_t S = (int64_t)GLYPH_MTFP_SCALE;
    ...
    for (int j = 0; j < cols; j++) {
        ...
        int64_t scaled = (num >= 0)
            ? (num + (int64_t)(GLYPH_MTFP_SCALE / 2)) / S
            : (num - (int64_t)(GLYPH_MTFP_SCALE / 2)) / S;
        int64_t result = scaled + (int64_t)bias[j];
        yi[j] = glyph_mtfp_clamp64(result);
    }
}

void glyph_mtfp_layernorm(
    glyph_mtfp_t* dst, const glyph_mtfp_t* src,
    const glyph_mtfp_t* weight, const glyph_mtfp_t* bias,
    glyph_mtfp_t eps, int rows, int cols)
{
...
#if GLYPH_HAS_DISPATCH
    if (rows >= GLYPH_SERIAL_ROW_THRESHOLD) {

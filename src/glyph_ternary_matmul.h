/*
 * glyph_ternary_matmul.h — MTFP activations × packed ternary weights → MTFP
 *
 * GLYPH IS TERNARY / MULTI-TRIT / MULTI-TRIT FLOATING POINT ONLY.
 *
 * This is glyph's forward matmul kernel for routed FFN and routed projection
 * layers. Activations are glyph_mtfp_t (int32 ternary fixed-point); weights
 * are 2-bit packed trits; output is glyph_mtfp_t. Inner loop is pure integer
...
#ifndef GLYPH_TERNARY_MATMUL_H
#define GLYPH_TERNARY_MATMUL_H
 ...
 * Kp = GLYPH_TRIT_PACKED_BYTES(K) bytes holding K trits LSB-first.
 *
 * Output is MTFP (int32 cells), accumulated internally in int64 and
 * clamped to ±GLYPH_MTFP_MAX_VAL on store.
 */
